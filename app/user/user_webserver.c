...
char user_web_page[USER_PAGE_SIZE] = {};
int silder1_num , silder2_num,silder3_num; 


LOCAL int parse_data(char*in,char delimiter){
	char str_len=0;
	char buf_1[1]={0}, buf_2[2]={0,0}, buf_3[3]={0,0,0} , buf_4[4] = {0};
	int temp=0;
	
	do{
		str_len++;
	}while(in[str_len]!=delimiter);
	
			switch(str_len){
				case(1):
						memcpy(buf_1,in,str_len);
						temp=atoi(buf_1);
						break;
				case(2):
						memcpy(buf_2,in,str_len);
						temp=atoi(buf_2);
						break;
				case(3):
						memcpy(buf_3,in,str_len);
						temp=atoi(buf_3);
						break;
				case(4):
						memcpy(buf_4,in,str_len);
						temp=atoi(buf_4);
						break;
			}
        ...
        switch (pURL_Frame->Type) {
            case GET:
                ...
                if (os_strcmp(pURL_Frame->pSelect, "client") == 0 &&
					...
					"};"
					"function updateVal() {"
					    "getEl(\'value1\').innerHTML=getEl(\'slider1\').value;"
					    "getEl(\'value2\').innerHTML=getEl(\'slider2\').value;"
					    "getEl(\'value3\').innerHTML=getEl(\'slider3\').value;"
					"}"
					"window.onload=updateVal;"
					"</script>"
					"</head>"
					"<body>"
					"<form action=\"\" target=\"tif\" id=\"form\" method=\"POST\">"
					"<div style=\"text-align:center; line-height:1em;\">"
					"<font size=\"12\">"
					"<p>"
					"<label for=\"amount1\">Vol1:</label><div id=\"value1\"></div>"
					"</p>"
					"<input type=\"range\" id=\"slider1\" style=\"width:400px; height: 100px;\" min=\"0\" max=\"1023\" value=\"512\" onchange=\"postVal();\" name=\"slider1\"/>"
					"<p>"
					"<label for=\"amount2\">Vol2:</label><div id=\"value2\"></div>"
					"</p>"
					"<input type=\"range\" id=\"slider2\" style=\"width:400px; height: 100px;\" min=\"0\" max=\"1023\" value=\"512\" onchange=\"postVal();\" name=\"slider2\"/>"
					"<p>"
					"<label for=\"amount3\">Vol3:</label><div id=\"value3\"></div>"
					"</p>"
					"<input type=\"range\" id=\"slider3\" style=\"width:400px; height: 100px;\" min=\"0\" max=\"1023\" value=\"512\" onchange=\"postVal();\" name=\"slider3\"/>"
					"</font>"
					...
					"</body>"
					"</html>\r\n");
    				espconn_sent(ptrespconn, user_web_page, os_strlen(user_web_page));
                }
            ...
            case POST:
        		...
        		//post data silder1=hoge&silder2=hage&silder3=fuge
	        	term = (char *)os_strstr( pusrdata, "slider1" ) ;
	        	
	        	if(term != NULL){
	        		silder1_num = parse_data(term+8, '&');
	        	}
				
        		term = (char *)os_strstr( pusrdata, "slider2" ) ;
	        	
	        	if(term != NULL){
	        		silder2_num = parse_data(term+8, '&');
	        	}
				
        		term = (char *)os_strstr( pusrdata, "slider3" ) ;
        	
        		if(term != NULL){
	        		silder3_num = parse_data(term+8, '&');
	        	}
        	
        		os_printf("silder1:%d, silder2:%d silder3:%d\n", silder1_num, silder2_num, silder3_num);
        		
        		silder1_num = silder1_num * 22;
        	
        		silder2_num = silder2_num * 22;
...
user_webserver_init(uint32 port)
{
   ...
   wifi_set_ip_info(SOFTAP_IF, &ipinfo);
	...
	//server_receive_body_buffer = NULL;
	//server_receive_body_buffer_size = 0;
...
#ifdef SERVER_SSL_ENABLE
    os_strcpy( ap_config.password, "ESP_Cerevo" );
#endif
    ap_config.channel = 5;				// Note: support 1 ~ 13
    ap_config.authmode = AUTH_OPEN;		// Note: Don't support AUTH_WEP in softAP mode.
    ap_config.ssid_hidden = 0;			// Note: default 0
    ap_config.max_connection = 1;		// Note: default 4, max 4
    ap_config.beacon_interval = 100;	// Note: support 100 ~ 60000 ms, default 100
